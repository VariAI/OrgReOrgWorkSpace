...
### 数据新鲜度约定(2026-07-23 立,适用于一切新功能)

客户端将打包为桌面端(Electron/Tauri),**没有"刷新页面"这个概念**——用户可能一周不重启。因此凡是新功能引入的"客户端持有的服务端数据",设计时必须三选一并在 PR 里写明:

1. **长连接 CMD 推送**(默认):服务端数据变更时 `SendCMD` 通知在线客户端,客户端订阅后按需增量重拉(带 version)。TSDD 既有模式(频道/群成员均如此)。
2. **打开即拉**:仅限每次进入界面都会重新请求的数据(资料卡、按需列表),陈旧窗口=界面停留时长,可接受。
3. **明确豁免**:一次性/极低频数据,注明理由。

反例(已立为待办):organization 模块零 SendCMD,通讯录只在挂载时拉一次。**"拉一次然后靠用户刷新"在本产品里不是一个合法选项。**
